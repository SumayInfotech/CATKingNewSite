...
(function megaMenus() {
  ...
  triggers.forEach(t => {
    ...
    // On touch devices, tap fires mouseenter THEN click. We need the click to NOT
    // close the panel that hover just opened. Suppress toggle for 350ms after a hover-open.
    t.addEventListener('click', function (e) {
      e.preventDefault();
      const justOpenedByHover = (Date.now() - lastOpenedAt) < 350 && panel?.classList.contains('open');
      if (justOpenedByHover) return; // panel was just opened by the same gesture, leave it open
      const wasOpen = panel?.classList.contains('open');
      if (wasOpen) {
        closeAll();
      } else {
        openPanel(target); // routes through the central function so aria-expanded stays in sync
      }
    });
  });
...
(function smoothAnchors() {
  document.querySelectorAll('a[href^="#"]').forEach(a => {
    ...
    a.addEventListener('click', function (e) {
      ...
      if (target) {
        e.preventDefault();
        const offset = 120; // top nav + sub nav
...
(function mobileDrawer() {
  ...
  // Close drawer when any nav link inside it is clicked (lets the navigation actually happen)
  drawer.querySelectorAll('a[href]').forEach(a => {
    a.addEventListener('click', function () {
      // Brief delay so the click navigates first; hide drawer afterwards
      setTimeout(closeDrawer, 50);
    });
  });

  // If viewport grows past mobile breakpoint while drawer is open, close it
